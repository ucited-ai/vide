...
- After frontend feature development or any user-visible frontend behavior change, the primary agent must run one integrated verification pass for each affected client surface after integrating the work:
  - Web: use the `test-vide-app` skill. Launch one isolated environment, authenticate through the printed pairing URL, and verify the affected flow in the controlled browser.
  - Mobile: use the `test-vide-mobile` skill. Connect one representative iOS Simulator or Android Emulator available on the host to one isolated environment and verify the affected flow. On compatible macOS hosts, prefer iOS for cross-platform changes and stream it through serve-sim in the Vide in-app browser or another available agent browser; use Android when it is the affected or viable platform.
...
- In a linked git worktree, dev state defaults to that worktree's gitignored `.vide`. This deliberately outranks an ambient `VIDE_HOME`, which could otherwise select the installed app's live `~/.vide/userdata` database. An explicit `--home-dir` still wins.
...
- `packages/shared`: Shared runtime utilities consumed by both server and client applications. Uses explicit subpath exports (e.g. `@vide/shared/git`) — no barrel index.
