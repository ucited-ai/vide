...
- Reads the shared production Vide Connect relay URL and Clerk client configuration before packaging clients.
...
- Publishes the CLI package (`apps/server`, npm package `vide`) with OIDC trusted publishing from the same workflow file:
...
```sh
vp run --filter vide-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `VIDE_WEB_ROUTER_URL`: defaults to `https://app.vide.local`.
- `VIDE_WEB_LATEST_DOMAIN`: defaults to `latest.app.vide.local`.
- `VIDE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.vide.local`.

Required Vercel domains:

- `app.vide.local`: the router domain users open, updated by stable releases.
- `latest.app.vide.local`: channel alias updated by stable releases.
- `nightly.app.vide.local`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__vide/channel?channel=latest` or
`/__vide/channel?channel=nightly`; the router stores the
`vide_web_channel` cookie and rewrites future requests on `app.vide.local` to
...
through `/__vide/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.vide.local` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `vide`) to the `nightly` npm dist-tag using the same nightly version.
- Does not commit version bumps back to `main`.

## Server self-update release invariant

Connected servers update to the client's exact version, not to an npm dist-tag. Every released
desktop or hosted client version must therefore have a matching `vide@<version>` package available on
...
For a release smoke test, confirm `npm view vide@<version> version` returns the expected version, then
...
- Repository slug source:
  - `VIDE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `VIDE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `vide` (or rename package first if needed).
...
2. Create an explicit App ID for `com.vide.app` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [Vide Connect Clerk Setup](../cloud/vide-connect-clerk.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.vide.app` and includes
    Associated Domains.
