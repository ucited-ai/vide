...
Vide has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.vide/userdata/logs/server.trace.ndjson`).
...
```bash
export VIDE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export VIDE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export VIDE_OTLP_SERVICE_NAME=vide-local
```

Optional:

```bash
export VIDE_TRACE_MIN_LEVEL=Info
export VIDE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `VIDE_OTLP_*`.

macOS app bundle example:

```bash
VIDE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
VIDE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
VIDE_OTLP_SERVICE_NAME=vide-desktop \
"/Applications/Vide.app/Contents/MacOS/Vide"
```
...
```bash
TRACE_FILE="${VIDE_HOME:-$HOME/.vide}/userdata/logs/server.trace.ndjson"
```
...
```bash
TRACE_FILE="$HOME/.vide/dev/logs/server.trace.ndjson"
tail -f "$TRACE_FILE"
```
...
- service name such as `vide-local`, `vide-dev`, or `vide-desktop`
...
- `vide_rpc_request_duration`
- `vide_orchestration_command_duration`
- `vide_orchestration_command_ack_duration`
- `vide_provider_turn_duration`
- `vide_git_command_duration`
- `vide_db_query_duration`

Counters tell you volume and failure rate:

- `vide_rpc_requests_total`
- `vide_orchestration_commands_total`
- `vide_provider_turns_total`
- `vide_git_commands_total`
- `vide_db_queries_total`
...
`vide_orchestration_command_ack_duration` measures:
...
1. Check `vide_orchestration_command_ack_duration` by `commandType`.
...
- `VIDE_OTLP_TRACES_URL` was not set
...
- `VIDE_TRACE_FILE`: override trace file path
- `VIDE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `VIDE_TRACE_MAX_FILES`: rotated file count, default `10`
- `VIDE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `VIDE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `VIDE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `VIDE_OTLP_TRACES_URL`: OTLP trace endpoint
- `VIDE_OTLP_METRICS_URL`: OTLP metric endpoint
- `VIDE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `VIDE_OTLP_SERVICE_NAME`: service name, default `vide-server`
