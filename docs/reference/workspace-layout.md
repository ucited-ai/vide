...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `vide` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@vide/shared/git`, `@vide/shared/DrainableWorker`) — no barrel index.
