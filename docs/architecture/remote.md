...
This document describes the target architecture for first-class remote environments in Vide.
...
- Keep the Vide server as the execution boundary.
...
Vide already has a clean runtime boundary: the client talks to a Vide server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Vide server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Vide server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Vide runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Vide server instance.
...
How does the client speak WebSocket to a Vide server?
...
- best fit for hosted or self-managed remote Vide deployments
...
For Vide, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a Vide-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote Vide server
...
How does a Vide server come to exist on the target machine?
...
The user or operator already runs Vide on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Vide:
...
The recommended Vide flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Vide availability.
3. Desktop launches or reuses a remote Vide server.
...
This is the inverse of remote launch: a local Vide server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Vide server might be reached through direct `wss`.
...
Vide already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Vide should not copy that part.

Vide already has the right runtime boundary:

- one Vide server per environment
- ordinary HTTP/WebSocket between client and environment

So Vide should borrow Zed's launch discipline, not its transport protocol.
