# Vide Connect Clerk Setup

Vide Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `vide-relay` template with the shared
`vide-relay` audience.

## Application Keys

Vide Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
VIDE_CLERK_PUBLISHABLE_KEY=<publishable key>
VIDE_CLERK_JWT_TEMPLATE=<JWT template name>
VIDE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
VIDE_RELAY_URL=https://relay.example.com
```
...
should set `VIDE_CLERK_PUBLISHABLE_KEY`, `VIDE_CLERK_JWT_TEMPLATE`,
`VIDE_CLERK_CLI_OAUTH_CLIENT_ID`, and `VIDE_RELAY_URL` before building. EAS preview and
...
absent, the `vide connect` CLI command group is omitted. The bundled server still accepts runtime
...
deployment defaults.
`vp run --filter vide-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `vide connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the Vide CLI.
...
5. Set `VIDE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
`vide connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `vide connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running Vide server. The next `vide serve` or `vide start` reconciles the relay link and launches the
managed tunnel. `vide connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `vide connect link` can re-enable exposure without another browser flow. `vide connect
...
logout leaves it running; manage it with `vide service status`, `install`, `update`, and `uninstall`.

The current OAuth callback listener binds to loopback port `34338`. When running the CLI over SSH,
forward that port before running `vide connect login` or `vide connect link`:
...
`CLERK_JWT_AUDIENCE=vide-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `VIDE_RELAY_URL` still
...
Local desktop development uses `vide-dev://app`, while packaged builds use `vide://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `vide-dev://app`; the production Clerk instance should only need
`vide://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.vide.app`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.vide.app` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.vide.app`.
...
```dotenv
VIDE_APPLE_TEAM_ID=ABC1234567
VIDE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/vide.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
VIDE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `VIDE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`VIDE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `VIDE_PORT` set. Rebuild the signed app
...
VIDE_PORT=13773 \
  "/Applications/Vide (Alpha).app/Contents/MacOS/Vide (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Vide (Alpha).app"
codesign -d --entitlements :- "/Applications/Vide (Alpha).app"
```
...
Approved signed-in users manage Vide Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the Vide Connect controls on the Connections page.

On mobile, signed-out users open **Settings > Vide Account** to reach `/settings/waitlist` within the
