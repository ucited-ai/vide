...
This guide is for people who want to use more than one Codex account in Vide.
...
- both accounts can see the same Vide/Codex sessions
...
In Vide Settings, name it something obvious:
...
In Vide Settings, add another Codex provider:
...
Vide shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Vide treats it
