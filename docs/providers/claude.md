...
This guide is for people who want to use more than one Claude setup in Vide.
...
In Vide Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means Vide uses your normal home directory.
...
Then add another Claude provider in Vide:
...
Vide only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Vide keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in Vide Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Vide stores the value as a server secret and does not
...
Vide does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in Vide.
