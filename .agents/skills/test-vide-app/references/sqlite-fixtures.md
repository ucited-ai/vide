...
Load this reference only when inspecting or seeding local Vide state directly.
...
```bash
node apps/server/scripts/vide-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/vide-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/vide-seed.sql
```
