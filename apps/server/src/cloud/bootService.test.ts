...
const makeTestContext = Effect.fn("test.makeTestContext")(function* () {
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "vide-boot-service-test-" });
  ...
  return {
    ...
    dirs: {
      home: root,
      baseDir: path.join(root, ".vide"),
      logsDir: path.join(root, ".vide", "userdata", "logs"),
      stableEntry,
    },
  };
});
...
it("renders a systemd unit with absolute paths and append-mode logging", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    videEntryPath: "/home/theo/.vide/runtime/versions/0.0.27/node_modules/vide/dist/bin.mjs",
    baseDir: "/home/theo/.vide",
    logPath: "/home/theo/.vide/userdata/logs/boot-service.log",
    unitPath: "/home/theo/.config/systemd/user/vide.service",
  });
  ...
  assert.equal(
    ...
    [
      ...
      "Type=simple",
      "WorkingDirectory=%h",
      "Environment=VIDE_HOME=/home/theo/.vide",
      "Environment=Vide_BOOT_SERVICE_UNIT=vide.service",
      "ExecStart=/usr/local/bin/node /home/theo/.vide/runtime/versions/0.0.27/node_modules/vide/dist/bin.mjs serve",
      "Restart=always",
      "RestartSec=5",
      "StandardOutput=append:/home/theo/.vide/userdata/logs/boot-service.log",
      "StandardError=append:/home/theo/.vide/userdata/logs/boot-service.log",
      "",
      "[Install]",
...
it("quotes systemd values containing spaces and escapes percent specifiers", () => {
  ...
  assert.equal(BootService.quoteSystemdValue("/home/me/Vide Data"), '"/home/me/Vide Data"');
  assert.equal(BootService.quoteSystemdValue("/opt/100%cpu"), "/opt/100%%cpu");

  const unit = BootService.renderBootServiceUnit({
    nodePath: "/home/me/my tools/node",
    videEntryPath: "/home/me/Vide Data/bin.mjs",
    baseDir: "/home/me/Vide Data",
    logPath: "/home/me/100%logs/boot.log",
    unitPath: "/home/me/.config/systemd/user/vide.service",
  });
  assert.include(unit, 'ExecStart="/home/me/my tools/node" "/home/me/Vide Data/bin.mjs" serve');
  assert.include(unit, 'Environment=VIDE_HOME="/home/me/Vide Data"');
...
it("flags package-manager cache entry points as ephemeral", () => {
  assert.isTrue(
    BootService.isEphemeralCacheEntry("/home/theo/.npm/_npx/abc123/node_modules/vide/dist/bin.mjs"),
  );
  assert.isTrue(
  ...
  assert.isTrue(
    BootService.isEphemeralCacheEntry(
      "/home/theo/.cache/pnpm/dlx/abc/node_modules/vide/dist/bin.mjs",
    ),
  );
  assert.isTrue(
  ...
  assert.isFalse(
    BootService.isEphemeralCacheEntry(
      "/home/theo/.vide/runtime/versions/0.0.27/node_modules/vide/dist/bin.mjs",
    ),
  );
});
...
it.layer(NodeServices.layer)("BootService", (it) => {
  ...
  it.effect("pins a runtime via npm install when running from the npx cache", () =>
    Effect.gen(function* () {
      ...
      const service = yield* BootService.make({
        ...
        logsDir: dirs.logsDir,
        cliVersion: "0.0.27",
        host: makeHost("/home/theo/.npm/_npx/abc/node_modules/vide/dist/bin.mjs"),
      }).pipe(Effect.provide(makeRecordingRunnerLayer(commands)), provideHostRefs(dirs.home));
      ...
      assert.equal(
        plan.videEntryPath,
        path.join(runtimeDir, "node_modules", "vide", "dist", "bin.mjs"),
      );
      assert.deepEqual(commands[0], {
        command: "npm",
        args: ["install", "--prefix", runtimeDir, "--no-fund", "--no-audit", "vide@0.0.27"],
      });
  ...
  it.effect("reinstalls a pinned runtime when its entry point is missing", () =>
    Effect.gen(function* () {
      ...
      const plan = yield* service.install;
      yield* fs.makeDirectory(path.dirname(plan.videEntryPath), { recursive: true });
      yield* fs.writeFileString(plan.videEntryPath, "#!/usr/bin/env node\n");
      yield* fs.remove(plan.videEntryPath);
      commands.length = 0;
  ...
  it.effect("reports an installed-but-stale unit so the lifecycle can offer a repair", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(
        ...
        "[Service]\nExecStart=/old/node /old/vide serve\n",
      );
  ...
  it.effect("fails on non-Linux platforms without touching the filesystem", () =>
    Effect.gen(function* () {
      ...
      const service = yield* BootService.make({
        ...
        logsDir: dirs.logsDir,
        cliVersion: "0.0.27",
        host: makeHost("/usr/local/lib/node_modules/vide/dist/bin.mjs"),
      }).pipe(
        Effect.provide(makeRecordingRunnerLayer(commands)),
      ...
      assert.isFalse(
        yield* fs.exists(path.join(dirs.home, ".config", "systemd", "user", "vide.service")),
      );
  ...
  it.effect("removes the unit file when an activation step fails", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        commands.some(
          ({ command, args }) =>
            command === "systemctl" && args.join(" ") === "--user disable --now vide.service",
        ),
      );
    }),
  );
  ...
  it.effect("restores the previous unit when a repair cannot activate", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        repairCommands.some(
          ({ command, args }) =>
            command === "systemctl" && args.join(" ") === "--user restart vide.service",
        ),
      );
    }),
  );
