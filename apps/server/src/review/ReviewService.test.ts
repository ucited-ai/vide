...
describe("ReviewService", () => {
  it.effect("rejects diff preview cwd outside the configured workspace roots", () =>
    Effect.gen(function* () {
      ...
      const workspaceRoot = yield* fs.makeTempDirectoryScoped({ prefix: "vide-review-workspace-" });
      const outsideRoot = yield* fs.makeTempDirectoryScoped({ prefix: "vide-review-outside-" });
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "vide-review-base-" });
