...
import { sanitizeBranchFragment } from "@vide/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@vide/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("VIDE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("VIDE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("VIDE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("VIDE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("vide/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `vide/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set VIDE_BITBUCKET_EMAIL and VIDE_BITBUCKET_API_TOKEN, or VIDE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
