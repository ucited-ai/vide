import { EnvironmentHttpApi } from "@vide/contracts";
...
import { disableTailscaleServe, ensureTailscaleServe } from "@vide/tailscale";

// Effect's default preemptive shutdown waits 20s before finalizing request scopes.
// Vide's primary transport is long-lived WebSocket RPC, whose Effect scope finalizer
...
// Named so the settings service can also be provided to layers that sit
// upstream of the core bundle — `UsageService` reads the configured provider
// homes. Reusing the same layer value keeps Effect's memoisation on one
// instance rather than building a second settings service.
...
const RuntimeDependenciesLive = RuntimeCoreDependenciesLive.pipe(
  ...
  Layer.provideMerge(UsageService.layer.pipe(Layer.provide(ServerSettingsLayerLive))),
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.forkScoped(
          Effect.sleep("250 millis").pipe(
            ...
            Effect.tap(() => Effect.logInfo("Vide Connect desired link reconciled on startup")),
            Effect.catch((cause) =>
              Effect.logWarning("Failed to reconcile Vide Connect desired link on startup", {
                cause,
              }),
            ),
          ),
