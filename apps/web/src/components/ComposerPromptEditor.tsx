      ...
      <div className="relative">
        <PlainTextPlugin
          contentEditable={
            <ContentEditable
              className={cn(
                "block max-h-50 min-h-17.5 w-full overflow-y-auto whitespace-pre-wrap wrap-break-word bg-transparent text-(length:--text-chat) leading-relaxed text-foreground focus:outline-none",
                className,
              )}
          ...
          placeholder={
            terminalContexts.length > 0 ? null : (
              <div className="pointer-events-none absolute inset-0 text-(length:--text-chat) leading-relaxed text-muted-foreground/35">
                {placeholder}
              </div>
            )
          }
...
export function ComposerPromptEditor({
  ...
  const initialConfig = useMemo<InitialConfigType>(
    () => ({
      namespace: "vide-composer-editor",
      editable: true,
