import { findErrorTraceId } from "@vide/client-runtime/errors";
import {
...
import type { RelayClientEnvironmentRecord } from "@vide/contracts/relay";
...
/**
 * The user's Vide Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Vide Connect.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Vide Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[vide-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
