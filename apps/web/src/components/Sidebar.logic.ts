...
import type { ContextMenuItem } from "@vide/contracts";
import type { SidebarProjectSortOrder, SidebarThreadSortOrder } from "@vide/contracts/settings";
import {
...
export function resolveThreadStatusPill(input: {
  ...
  if (thread.hasPendingApprovals) {
    return {
      ...
      colorClass: "text-(--thread-status-attention)",
      dotClass: "bg-(--thread-status-attention)",
      pulse: false,
    };
  }
  ...
  if (thread.session?.status === "running") {
    return {
      ...
      dotClass: "bg-(--thread-status-active)",
      pulse: true,
    };
  }
  ...
  if (hasUnseenCompletion(thread)) {
    return {
      label: "Completed",
      colorClass: "text-(--thread-status-done)",
      dotClass: "bg-(--thread-status-done)",
      pulse: false,
    };
  }
