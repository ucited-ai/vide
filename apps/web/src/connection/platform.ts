...
} from "@vide/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@vide/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@vide/client-runtime/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@vide/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@vide/client-runtime/rpc";
import {
...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Vide Desktop" : "Vide Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Vide Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Vide Connect session is unavailable.",
          });
        }
        return token;
      }),
