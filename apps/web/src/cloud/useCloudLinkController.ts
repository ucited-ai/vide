...
/**
 * Drives the primary environment's Vide Connect link. Vide Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    ...
    toastManager.add({
      type: "error",
      title: "Could not update Vide Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Vide Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Vide Connect before enabling this."));
        return false;
      }
