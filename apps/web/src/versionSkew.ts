import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@vide/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "vide:version-mismatch-dismissals:v1";
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Vide version.",
  };
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the Vide desktop app on its machine — update the desktop app there to sync them.`;
    default:
