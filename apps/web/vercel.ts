...
const ROUTER_HOST = "app.vide.local";
const HOSTED_WEB_CHANNEL_COOKIE = "vide_web_channel";
const LATEST_ORIGIN = "https://latest.app.vide.local";
const NIGHTLY_ORIGIN = "https://nightly.app.vide.local";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @vide/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --filter '@vide/scripts...' --filter '@vide/web...'",
  routes: [
    {
      src: "/__vide/channel",
