...
import * as SshAuth from "@vide/ssh/auth";
import { discoverSshHosts } from "@vide/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@vide/ssh/errors";
import * as SshTunnel from "@vide/ssh/tunnel";
...
>()("@vide/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteVideRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Vide window is not available for SSH authentication.";
      break;
