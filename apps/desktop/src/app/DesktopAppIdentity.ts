...
const AppPackageMetadata = Schema.Struct({
  videCommitHash: Schema.optional(Schema.String),
});
...
>()("@vide/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.videCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
