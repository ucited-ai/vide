// This file mostly exists because we want dev mode to say "Vide (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Vide (Dev)" : "Vide (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.vide.app.dev.${devBundleIdSuffix || "local"}`
  : "com.vide.app";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["vide-dev"] : ["vide"];
...
// oxlint-disable-next-line vide/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["VIDE_PORT", environment.VIDE_PORT],
    ["VIDE_HOME", environment.VIDE_HOME],
    ["VIDE_COMMIT_HASH", environment.VIDE_COMMIT_HASH],
    ["VIDE_OTLP_TRACES_URL", environment.VIDE_OTLP_TRACES_URL],
    ["VIDE_OTLP_EXPORT_INTERVAL_MS", environment.VIDE_OTLP_EXPORT_INTERVAL_MS],
    ["VIDE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --vide-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Vide name.
