...
const remoteDebuggingPort = process.env.VIDE_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line vide/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.VIDE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.VIDE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--vide-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--vide-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
