...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    VIDE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    VIDE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    VIDE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    VIDE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    VIDE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    VIDE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("VIDE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("VIDE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "VideRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Vide relay through Alchemy."));
