...
describe("Clerk relay auth", () => {
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.vide.local/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(isAllowedClerkFrontendApiHostname("clerk.vide.local", "clerk.vide.local")).toBe(true);
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.vide.local")).toBe(false);
    expect(isAllowedClerkFrontendApiHostname("nested.clerk.vide.local", "clerk.vide.local")).toBe(
      false,
    );
  });
});
