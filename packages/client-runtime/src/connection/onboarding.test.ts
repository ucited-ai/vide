import { AuthStandardClientScopes, EnvironmentId } from "@vide/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/vide/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/vide/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("Vide Test");
    }),
  );
