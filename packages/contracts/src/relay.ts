...
}).annotate({ description: "Links an authenticated cloud user to a Vide environment." });
...
>()("@vide/contracts/relay/RelayClientPrincipal") {}
...
>()("@vide/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Vide Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["vide-mobile", "vide-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "vide-mobile" as const;
export const RelayWebClientId = "vide-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Vide Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Vide environments, connecting authorized clients, and publishing agent activity.",
  );
