...
describe("ssh tunnel scripts", () => {
  it("builds the remote vide runner with npx and npm fallbacks", () => {
    const script = buildRemoteVideRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "Vide_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec vide "$@"');
    assert.include(script, "exec npx --yes 'vide@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'vide@latest' -- \"$@\"");
    assert.include(script, "could not install 'vide@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `Vide_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for Vide_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteVideRunnerScript();

    assert.include(script, "Vide_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote vide runner", () => {
    const script = buildRemoteVideRunnerScript({
      packageSpec: "vide@nightly; touch /tmp/vide-owned",
    });

    assert.include(script, "exec npx --yes 'vide@nightly; touch /tmp/vide-owned' \"$@\"");
    assert.include(script, "exec npm exec --yes 'vide@nightly; touch /tmp/vide-owned' -- \"$@\"");
    assert.notInclude(script, "exec npx --yes vide@nightly; touch /tmp/vide-owned");
  });

  it("builds the remote vide runner with a node script override", () => {
    const script = buildRemoteVideRunnerScript({
    ...
    assert.include(
      script,
      "Vide_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$Vide_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote vide runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `Vide_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote Vide server did not become ready");
    assert.include(buildRemoteLaunchScript({ packageSpec: "vide@nightly" }), "vide@nightly");
    assert.include(
    ...
    assert.include(buildRemotePairingScript(target, { packageSpec: "vide@nightly" }), "vide@nightly");
    assert.include(
