...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@vide/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "vide-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when Vide re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# Vide_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${Vide_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$Vide_SSH_AUTH_SECRET"
  exit 0
fi
printf 'Vide ssh-askpass invoked without Vide_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Vide re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if Vide_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:Vide_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:Vide_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("Vide ssh-askpass invoked without Vide_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "vide-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined ? {} : { Vide_SSH_AUTH_SECRET: input.authSecret ?? "" }),
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "vide" }),
  };
});
