...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@vide/contracts";
...
const PUBLISHABLE_Vide_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteVideCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_Vide_VERSION_PATTERN.test(appVersion)) {
    return `vide@${appVersion}`;
  }
  ...
  return input.updateChannel === "nightly" ? "vide@nightly" : "vide@latest";
}
