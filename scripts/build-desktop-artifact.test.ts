...
import { HostProcessArchitecture, HostProcessPlatform } from "@vide/shared/hostProcess";
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Vide (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Vide (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                VIDE_DESKTOP_UPDATE_REPOSITORY: "pingdotgg/vide",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "pingdotgg/vide",
              },
            }),
          ),
        ),
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@vide/contracts": "workspace:*",
          "@vide/shared": "workspace:*",
          "@vide/ssh": "workspace:*",
          "@vide/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      VIDE_MACOS_PROVISIONING_PROFILE: "/tmp/vide.provisionprofile",
      VIDE_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });
    ...
    assert.deepStrictEqual(configuration, {
      ...
      provisioningProfilePath: "/tmp/vide.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.vide.app</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      VIDE_APPLE_TEAM_ID: "ABC1234567",
      VIDE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "VIDE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      VIDE_APPLE_TEAM_ID: "ABC1234567",
      VIDE_MACOS_PROVISIONING_PROFILE: "/tmp/vide.provisionprofile",
      VIDE_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          VIDE_APPLE_TEAM_ID: "ABC1234567",
          VIDE_MACOS_PROVISIONING_PROFILE: "/tmp/vide.provisionprofile",
          VIDE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      VIDE_APPLE_TEAM_ID: "ABC1234567",
      VIDE_MACOS_PROVISIONING_PROFILE: "/tmp/vide.provisionprofile",
      VIDE_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "VIDE_CLERK_PUBLISHABLE_KEY is invalid.");
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                VIDE_DESKTOP_SKIP_BUILD: "true",
                VIDE_DESKTOP_KEEP_STAGE: "true",
                VIDE_DESKTOP_SIGNED: "true",
                VIDE_DESKTOP_VERBOSE: "true",
                VIDE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
