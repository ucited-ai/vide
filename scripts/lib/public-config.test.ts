...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.VIDE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.VIDE_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.VIDE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VIDE_RELAY_URL).toBeUndefined();
    expect(env.VITE_VIDE_RELAY_URL).toBeUndefined();
    expect(env.VIDE_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.VIDE_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.VIDE_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.VIDE_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.VIDE_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.VIDE_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "VIDE_CLERK_PUBLISHABLE_KEY=pk_root\nVIDE_CLERK_JWT_TEMPLATE=template_root\nVIDE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nVIDE_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "VIDE_CLERK_PUBLISHABLE_KEY=pk_local\nVIDE_CLERK_JWT_TEMPLATE=template_local\nVIDE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nVIDE_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).VIDE_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          VIDE_CLERK_PUBLISHABLE_KEY: "pk_ci",
          VIDE_CLERK_JWT_TEMPLATE: "template_ci",
          VIDE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          VIDE_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      VIDE_RELAY_URL: "https://ci.example.test",
      VITE_VIDE_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        VIDE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_VIDE_RELAY_URL: "https://legacy.example.test",
    ...
    expect(
      loadRepoEnv({
        baseEnv: {
          ...
          VIDE_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          VIDE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
    ...
    expect(
      loadRepoEnv({
        baseEnv: {
          ...
          VIDE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          VIDE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      VIDE_RELAY_URL: "https://relay.example.test",
      VITE_VIDE_RELAY_URL: "https://relay.example.test",
      VIDE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      VIDE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      VIDE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "vide-public-config-"));
