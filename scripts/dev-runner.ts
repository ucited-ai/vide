...
import { resolveGitWorktreePath, resolveWorktreeVideHome } from "@vide/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@vide/shared/hostProcess";
...
export const DEFAULT_Vide_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".vide"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@vide/contracts",
    "--filter=@vide/web",
    "--filter=vide",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=vide", "dev"],
  "dev:web": ["run", "--filter=@vide/web", "dev"],
  "dev:desktop": ["run", "--filter=@vide/desktop", "--filter=@vide/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`VIDE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("VIDE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("VIDE_PORT_OFFSET"),
  devInstance: optionalStringConfig("VIDE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `VIDE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric VIDE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed VIDE_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_Vide_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly videHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .vide > ambient VIDE_HOME) is resolved
    // by the caller; an unset videHome here genuinely means "use the default".
    const configuredBaseDir = videHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.VIDE_HOME = resolvedBaseDir;
    } else {
      delete output.VIDE_HOME;
    }

    if (!isDesktopMode) {
      output.VIDE_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.VIDE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.VIDE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.VIDE_PORT = String(serverPort);
      ...
      delete output.VIDE_SINGLE_ORIGIN_DEV;
      delete output.VIDE_MODE;
      delete output.VIDE_NO_BROWSER;
      delete output.VIDE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.VIDE_HOST = host;
    }

    if (!isDesktopMode) {
      output.VIDE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.VIDE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.VIDE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.VIDE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.VIDE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.VIDE_MODE = "web";
      delete output.VIDE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`VIDE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["VIDE_PORT_OFFSET", "VIDE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.vide` — see @vide/shared/devHome for why this must
    // outrank an ambient VIDE_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeVideHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedVideHome =
      (input.videHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.VIDE_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      videHome: resolvedVideHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.VIDE_HOME ?? (yield* DEFAULT_Vide_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.VIDE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  videHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Vide data directory; runtime state is stored under userdata (equivalent to VIDE_HOME). Inside a git worktree this defaults to that worktree's own .vide so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to VIDE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("VIDE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to VIDE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("VIDE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to VIDE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("VIDE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to VIDE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("VIDE_PORT")),
  ),
